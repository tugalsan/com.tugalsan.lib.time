...
public class TS_LibTimeUtils {
    ...
    public static TGS_Optional<TGS_Time> fetch(CharSequence ip, int sslPort) {
        ...
        if (TGS_StringUtils.isNullOrEmpty(urlData)) {
            return TGS_Optional.ofEmpty("ERROR @ TS_LibTimeUtils.fetch", "REASON: TGS_StringUtils.isNullOrEmpty(urlData)");
        }
        var pattern = TGS_LibTime.PATTERN();
        if (pattern.length() != urlData.length()) {
            return TGS_Optional.ofEmpty("ERROR @ TS_LibTimeUtils.fetch", "REASON: pattern.length() != urlData.length() where", "pattern", pattern);
        }
        var st = new StringTokenizer(" ");
        if (st.countTokens() != 2) {
            return TGS_Optional.ofEmpty("ERROR @ TS_LibTimeUtils.fetch", "REASON: st.countTokens() != 2 where", "pattern", pattern, "urlData", urlData);
        }
        var dateStr = st.nextToken();
        var dateLng = TGS_CastUtils.toLong(dateStr);
        if (dateLng == null) {
            return TGS_Optional.ofEmpty("ERROR @ TS_LibTimeUtils.fetch", "REASON: dateLng == null where", "pattern", pattern, "urlData", urlData, "dateStr", dateStr);
        }
        var timeStr = st.nextToken();
        var timeLng = TGS_CastUtils.toLong(timeStr);
        if (timeLng == null) {
            return TGS_Optional.ofEmpty("ERROR @ TS_LibTimeUtils.fetch", "REASON: dateLng == null where", "pattern", pattern, "urlData", urlData, "dateStr", dateStr, "timeStr", timeStr);
        }
